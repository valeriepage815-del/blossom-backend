...
const DRY_RUN = false; // <-- flip to false when you're 100% ready
...
// Limit how many pairs to process per run
const MAX_PAIRS_PER_RUN = 2; // gentler on Auth0 API
let pairsToProcess = pairs.slice(0, MAX_PAIRS_PER_RUN);
...
(async () => {
  ...
  // Track which pairs were successfully processed
  ...
  for (let i = 0; i < pairsToProcess.length; i++) {
    ...
    try {
      ...
      // If secondary user is already deleted, treat as processed
  ...
  // Remove processed pairs from pairs and write back to CSV
  if (processedIndexes.length > 0) {
    // Remove from pairs (not pairsToProcess, so future runs skip them)
    pairs = pairs.filter((_, idx) => !processedIndexes.includes(idx));
    // Write updated pairs back to CSV
    const newLines = [header, ...pairs.map(pair => JSON.stringify([
      pair.email,
      pair.primary_id,
      pair.primary_connection,
      pair.primary_has_app_metadata,
      pair.secondary_id,
      pair.secondary_connection,
      pair.secondary_has_app_metadata
    ]).slice(1, -1))].join("\n");
    fs.writeFileSync(csvPath, newLines + "\n", "utf8");
    console.log(`\n📝 Removed ${processedIndexes.length} processed pair(s) from CSV.`);
  }
